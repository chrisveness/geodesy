/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
/*  Geodesy Test Harness - latlon-spherical                           (c) Chris Veness 2014-2015  */
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
...
describe('latlon-spherical', function() {
    ...
    describe('formatting', function() {
        test('toString d',       function() { LatLon(51.521470, -0.138833).toString('d', 6).should.equal('51.521470°N, 000.138833°W'); });
        test('toString dms',     function() { LatLon(51.521470, -0.138833).toString('dms', 2).should.equal('51°31′17.29″N, 000°08′19.80″W'); });
    });
    ...
    describe('geodesics', function() {
        ...
        test('Clairaut 0°',      function() { LatLon(0,0).maxLatitude( 0).should.equal(90); });
        test('Clairaut 1°',      function() { LatLon(0,0).maxLatitude( 1).should.equal(89); });
        test('Clairaut 90°',     function() { LatLon(0,0).maxLatitude(90).should.equal(0); });

        var parallels = LatLon.crossingParallels(LatLon(0,0), LatLon(60,30), 30);
        test('parallels 1',      function() { LatLon(30, parallels.lon1).toString().should.equal('30°00′00″N, 009°35′39″E'); });
        test('parallels 2',      function() { LatLon(30, parallels.lon2).toString().should.equal('30°00′00″N, 170°24′21″E'); });
    });
